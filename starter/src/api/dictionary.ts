...
try {
  ...
  console.warn('Chinese lexicon library not available in browser, using mock data')
}
...
const mockDictionary: Record<string, DictionaryEntry[]> = {
  '你': [{
    ...
    searchablePinyin: 'ni',
    simpEtymology: {
      notes: 'Phonosemantic compound. 人 represents the meaning and 尔 represents the sound.',
      definition: 'you',
      components: [
        { type: 'meaning', char: '人' },
        { type: 'sound', char: '尔' }
  ...
  '好': [{
    simp: '好',
    trad: '好',
    definitions: ['good', 'well', 'proper', 'good to', 'easy to', 'very', 'so', '(suffix indicating completion or readiness)'],
    pinyin: 'hǎo',
...
export async function searchDictionary({ query, type, limit = 10 }: SearchParams): Promise<DictionaryEntry[]> {
  try {
...
export async function getCharacterData(character: string): Promise<CharacterData> {
  try {
    ...
    if (mockDictionary[character]) {
      ...
      return {
        ...
        entries,
        radicals: Array.from(radicals),
        strokeCount: character === '你' ? 7 : character === '好' ? 6 : character === '我' ? 7 : 0,
        components: Array.from(radicals)
      }
    }
...
export function getShortDefinition(word: string, pinyin?: string): string {
  try {
    ...
    // Fall back to mock data
    if (mockDictionary[word] && mockDictionary[word][0]) {
      return mockDictionary[word][0].definitions[0] || ''
    }
    
    return ''
  } catch (error) {
    console.error('Error getting gloss:', error)
    return ''
  }
}